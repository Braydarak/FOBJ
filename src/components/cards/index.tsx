...
const Card: React.FC<CardProps> = ({ title, obj, amount, onClick }) => {
  return (
    <div className="relative w-[150px] h-[140px] bg-cardBG card-shadow rounded-[30px] md:w-[250px] md:h-[200px] flex justify-center items-center md:text-[40px] font-medium cursor-pointer overflow-hidden"  onClick={onClick}>
      <div className="w-full h-full flex justify-center items-center opacity-100 transition-opacity duration-300 hover:opacity-0">
        <span className="pointer-events uppercase">{title}</span>
      </div>
